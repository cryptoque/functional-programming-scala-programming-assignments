...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def timesHelper(chars:List[Char]): List[List[Char]] = {
        chars match {
          case List() => List()
          case x :: y =>
            val (a, b) = (x::y).partition(_ == x)
            a :: timesHelper(b)
        }
      }
      timesHelper(chars) map (x =>
        (x.head, x.length))
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      val newfreqs = freqs.sortWith(_._2 < _._2)
      newfreqs.map(x => Leaf(x._1, x._2))
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      ...
      case head :: tails => tails match {
        case Nil => List(head)
        case a::b => (makeCodeTree (head, a) :: b) sortWith ((a, b)=> weight(a)< weight(b))
      }
    }
   ...
   */
    def until(singleton: List[CodeTree] => Boolean, combine: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree]  = trees match {
      case x if (singleton(x) == true) => x
      case x =>
        val newtree = combine(x)
        until(singleton, combine)(newtree)
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
      case Nil => List()
      case a::b => if(a._1 == char) a._2
      else codeBits(b)(char)
      // case _ => List()
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      case Leaf(c, _) => List()
      case Fork(_, _ , charsList, _) =>
        // val encoded = encode(tree)(charsList)
        // println(encoded)
        charsList.map { char =>
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      text.map { char =>
        ...
        println(codeBits(convert(tree))(char))
        codeBits(convert(tree))(char)
      }.flatten
    }
  }
