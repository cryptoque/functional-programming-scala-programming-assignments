...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
    *
    * This is implemented here!! because filter method is same for both Empty and nonEmpty TweetSets
   */
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(x => (unparseToData(List(x)) contains google.foreach(y => y)))
  lazy val appleTweets: TweetSet = allTweets.filter(x => (unparseToData(List(x)) contains google.foreach(y => y)))
  println(googleTweets.toString)
